...
class MatchingEngine:
    ...
    def match_item(self, extracted_text: str) -> tuple:
        ...
        First, enforces strict `matching_rules` generated by the LLM (if any).
        ...
        for item_id, item_data in self.catalog.items():
            ...
            rules = item_data.get("matching_rules", {})
            
            if rules:
                # 1. Check exclusions
                must_not = rules.get("must_not_contain_regex", [])
                failed_not = False
                ...
                # 2. Check strict inclusions
                must = rules.get("must_contain_regex", [])
                failed_must = False
                for pattern in must:
                    try:
                ...
                # Enhance the fuzzy matching description internally using LLM keywords if present
                keywords = rules.get("keywords", [])
                if keywords:
                    desc = desc + " " + " ".join(keywords)

            # Survived strict rules (or legacy item without rules)
